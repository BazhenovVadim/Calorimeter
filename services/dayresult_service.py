...
class DayResultService:
    ...
    async def get_result_by_user_id(user_id:int, session: AsyncSession) -> List[DayResults]:
        ...
        day_result = query.scalars().all()
        return day_result

    @staticmethod
    async def get_all_results(session:AsyncSession) -> List[DayResults]:
        query = await session.execute(select(DayResults).order_by(DayResults.id_result))
